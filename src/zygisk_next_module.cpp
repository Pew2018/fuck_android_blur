...
using SetBlurFn = void* (*)(void* transaction, const void* surfaceControlSp, int radius);
using GetNameFn = const std::string& (*)(const void* surfaceControl);

const ZygiskNextAPI* gApi = nullptr;
SetBlurFn gOriginalSetBlur = nullptr;
GetNameFn gGetName = nullptr;
std::string gProcess;
...
void* hookedSetBlur(void* transaction, const void* surfaceControlSp, int radius) {
    ...
    if (radius > 0 &&
        ...
        if (propBool("persist.sys.pixelblur.debug", false)) {
            ...
            if (surfaceControl && gGetName) {
                const std::string& name = gGetName(surfaceControl);
                logLine("%s: radius %d -> 0 on %s",
                        gProcess.c_str(), radius, name.c_str());
            } else {
                logLine("%s: radius %d -> 0",
...
void hookInTargetProcess() {
    ...
    if (!resolver) {
        __android_log_print(ANDROID_LOG_ERROR, kTag,
                            "Could not create libgui symbol resolver");
        return;
    }
    ...
    void* target =
            gApi->symbolLookup(resolver, kSetBlurSymbol, false, nullptr);

    if (!target) {
        target = gApi->symbolLookup(
                resolver, "setBackgroundBlurRadius", true, nullptr);
        if (target) logLine("Using prefix-resolved setBackgroundBlurRadius symbol");
    }

    void* nameFn =
            gApi->symbolLookup(resolver, kGetNameSymbol, false, nullptr);
    if (nameFn) gGetName = reinterpret_cast<GetNameFn>(nameFn);
    ...
    logLine("Native blur hook active in %s", gProcess.c_str());
    gApi->freeSymbolResolver(resolver);
}
...
void onModuleLoaded(void*, const ZygiskNextAPI* api) {
    ...
    logLine("Loaded into target process: %s", gProcess.c_str());
    hookInTargetProcess();
}
