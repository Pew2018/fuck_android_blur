...
if [[ -z "${ANDROID_NDK_HOME:-}" || ! -x "${ANDROID_NDK_HOME}/build/cmake/android.toolchain.cmake" ]]; then
  echo "Android NDK not found." >&2
  exit 1
fi
API_HEADER="$ROOT/third_party/zygisk_next_api.h"
mkdir -p "$ROOT/third_party"
curl -fsSL https://raw.githubusercontent.com/Dr-TSNG/ZygiskNext/main/zygisk_next_api.h -o "$API_HEADER"
BUILD="$ROOT/.build"
rm -rf "$BUILD"
...
rm -f "$OUT"
(cd "$ROOT" && zip -r -9 "$OUT" module.prop zn_modules.txt service.sh action.sh webroot lib >/dev/null)
echo "Built: $OUT"
